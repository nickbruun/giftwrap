...
class ControlFileGenerator(object):
    ...
    Follows the documentation of http://www.debian.org/doc/debian-policy/ch-controlfields.html
    ...
    def generate(self, 
        ...
        if isinstance(package.architectures, list):
            ...
            host_architecture = run(['dpkg', '--print-architecture'], 
                                    vital = False)
            architectures = [host_architecture.strip() if not host_architecture is None else 'any']

        if ((not control_binary) and 
            (not 'source' in architectures)):
        ...
        rules = {
            ...
            'Description': '%s\n %s' % (string_ending_in_period(package.description), 
                                        string_ending_in_period(package.long_description))
        }
        
        if not package.homepage is None:
            rules['Homepage'] = package.homepage
        
        if control_binary:
            rules['Package'] = package.name
        
        if not package.section is None:
            rules['Section'] = package.section
        
        if not package.priority is None:
