...
class PackagingContext(object):
    ...
    def __init__(self):
        ...
        self._data_path = os.path.join(self._root_path,
                                       'data')
    ...
    def control_path(self, 
        ...
        return os.path.join(self._control_path, 
                            filename)
    ...
    def _mkdirp(self, 
        ...
        :param permissions: Permissions to apply to any directories below the 
        data path.
        """
        
        from utilities import mkdirp
        mkdirp(path)
    ...
    def make_data_dir(self, 
        ...
        if user != None:
            self.postinst_commands += ['chown -R %s %s' % (('%s:%s' % (user, group) if group != None else user), 
                                                           path)]
    ...
    def _write_postinst(self, 
                        package):
        """Write the postinst file during the build process.
        """
        
        from utilities import write_lines_to_file
    ...
    def _write_control(self, 
        ...
        from utilities import write_lines_to_file, run, string_ending_in_period
    ...
    def _write_copyright(self, 
        ...
        write_to_file(package.copyright, 
                      self.data_path('/usr/share/doc/%s/copyright' % (package.name)))
    ...
    def _write_conffiles(self, 
        ...
        for root, sub_folders, rel_files in os.walk(etc_path):
            for rel_file in rel_files:
                conffiles.append('/etc/%s' % os.path.relpath(os.path.join(root, 
                                                                          rel_file), 
                                                             etc_path))
        
        # Write the configuration files to the control file.
        from utilities import write_lines_to_file
        write_lines_to_file(conffiles, 
                            self.control_path('conffiles'))
    ...
    def pack(self, 
        ...
        from utilities import ar_create, targz, run, glob_dir
        
        data_tarball_path = os.path.join(self._root_path,
                                         'data.tar.gz')
        control_tarball_path = os.path.join(self._root_path,
                                            'control.tar.gz')
        ...
        debian_binary_path = os.path.join(self._root_path,
                                          'debian-binary')
        ...
        # - the debian-binary file must go before all others. http://ubuntuforums.org/archive/index.php/t-1481153.html
