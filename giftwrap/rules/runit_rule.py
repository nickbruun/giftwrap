...
class RunItRule(Rule):
    ...
    :ivar name: Identifier name. Used for both the service and the logging 
    directory.
    :ivar args: Application execution arguments like for subprocess 
    invocations.
    ...
    def __init__(self, 
        ...
        :param name: Identifier name. Used for both the service and the logging 
        directory.
        :param args: Application execution arguments. If a :class:`str` 
        instance is supplied, it is split if it contains whitespaces, or 
        simply reated as the command.
        :param user: default `root`, Username to execute an application as.
        :ivar group: default ``None``, Group name to execute an application 
        under.
    ...
    def apply(self, 
        ...
        write_lines_to_file(run_lines,
                            context.data_path('/etc/service/%s/run' % (self._name)), 
                            permissions = 0755)
        write_lines_to_file(log_run_lines,
                            context.data_path('/etc/service/%s/log/run' % (self._name)), 
                            permissions = 0755)
